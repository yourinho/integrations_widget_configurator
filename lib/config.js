 ...
 * Region and language options come from VariantConfig (variant-config.js).
 */

const DEFAULT_WIDGET_URL = 'https://yourinho.github.io/integrations_widget/albato-widget-global.iife.js';

/** Region value → regions array for initWidget. */
function regionValueToRegions(value) {
  if (value === 'all') return [];
  if (value === 'brazil') return [2];
  if (value === 'global') return [3];
  if (value === 'brazil-global') return [2, 3];
  return [];
}

/** Regions array → region select value. */
function regionsToRegionValue(regions) {
  if (!regions || regions.length === 0) return 'all';
  if (regions.length === 2 && regions.includes(2) && regions.includes(3)) return 'brazil-global';
  if (regions.length === 1 && regions[0] === 2) return 'brazil';
  if (regions.length === 1 && regions[0] === 3) return 'global';
  return 'all';
}

/** Supported languages and default from VariantConfig, fallback = default variant (no CIS/RU). */
const SUPPORTED_LANGUAGES = (typeof window.VariantConfig !== 'undefined')
  ? window.VariantConfig.getSupportedLanguages()
  : [
...
const DEFAULT_LANGUAGE = (typeof window.VariantConfig !== 'undefined')
  ? window.VariantConfig.getDefaultLanguage()
  : 'en';
...
function getDefaultConfig() {
  const defaultRegion = (typeof window.VariantConfig !== 'undefined')
    ...
    : 'brazil-global';
  const regions = regionValueToRegions(defaultRegion);
  return {
...
window.WidgetConfig = {
  getDefaultConfig,
  getInitWidgetParams,
  regionValueToRegions,
  regionsToRegionValue,
  DEFAULT_WIDGET_URL,
  DEFAULT_COLORS,
  DEFAULT_TYPOGRAPHY,
  DEFAULT_TEXTS,
